...
def main(args):
    ...
    smdim = np.random.randint(2000, 3000)
    lgdim = np.random.randint(6000, 7000)
    ...
    tests = [
        ...
        [ EOMTestCase, (h2o2_molstr, 'hydrogen-peroxide','6-31G'),dict(no_eigs=4, max_vecs=100, maxiter=300, e_conv=1.0e-6, r_conv=1.0e-5)],
        # "smaller" tests 2000-5000 dimension with varying sparsity (1-5 roots)
        [ NumpyRandomTestCase,(smdim, 1000), dict(no_eigs=np.random.randint(3, 5), max_vecs=100, maxiter=300, e_conv=1.0e-6) ],
        [ NumpyRandomTestCase,(smdim, 100), dict(no_eigs=np.random.randint(3, 5), max_vecs=100, maxiter=300, e_conv=1.0e-6) ],
        [ NumpyRandomTestCase,(smdim, 10), dict(no_eigs=np.random.randint(3, 5), max_vecs=100, maxiter=300, e_conv=1.0e-6) ],
        # Stress test, no sparsity factor means non-ideal problem, but given
        # enough iters it *should* work
        [ NumpyRandomTestCase,(smdim, 1), dict(no_eigs=np.random.randint(3, 5), max_vecs=100, maxiter=3000, e_conv=1.0e-6) ],
        # Stress test, very tight convergence, see how many iters needed
        [ NumpyRandomTestCase,(smdim, 1000), dict(no_eigs=1, max_vecs=100, maxiter=3000, e_conv=1.0e-10) ],
        # # Stress test, very larger number of roots
        [ NumpyRandomTestCase,(smdim, 1000), dict(no_eigs=smdim//100, max_vecs=smdim//10, maxiter=3000, e_conv=1.0e-5, r_conv=1.0e-4) ],
        ]  # yapf: disable
    testing_results = []
    for test_class, case_args, case_kwargs in tests:
        if (test_class == EOMTestCase) and args.skip_eom:
            continue
        elif (test_class == NumpyRandomTestCase) and args.skip_random:
            continue
        else:
